...
async def test_photo_handler_openai_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    context = cast(
        CallbackContext[Any, dict[str, Any], dict[str, Any], dict[str, Any]],
        SimpleNamespace(bot=bot, user_data=MappingProxyType(user_data)),
    )
    ...
    user_data_err = photo_handlers._get_mutable_user_data(context)
...
async def test_photo_handler_fallback_parse_fail(
    ...
    user_data_fp = photo_handlers._get_mutable_user_data(context)
...
async def test_photo_handler_run_timeout(
    ...
    user_data_timeout = {"thread_id": "tid"}
    context = cast(
        CallbackContext[Any, dict[str, Any], dict[str, Any], dict[str, Any]],
        SimpleNamespace(bot=bot, user_data=MappingProxyType(user_data_timeout)),
    )
    monkeypatch.chdir(tmp_path)
    result = await photo_handlers.photo_handler(update, context)
    ...
    user_data_timeout = photo_handlers._get_mutable_user_data(context)
...
async def test_photo_handler_typing_action_error(
    ...
    user_data_typing = photo_handlers._get_mutable_user_data(context)
...
async def test_photo_handler_value_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    context = cast(
        CallbackContext[Any, dict[str, Any], dict[str, Any], dict[str, Any]],
        SimpleNamespace(user_data=MappingProxyType(user_data)),
    )
    ...
    user_data_val = photo_handlers._get_mutable_user_data(context)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (dict(message=None), dict(user_data=MappingProxyType({}), _user_data={})),
        (
            dict(
                message=SimpleNamespace(
                    document=SimpleNamespace(
            ...
            dict(user_data=MappingProxyType({}), _user_data={}),
        ),
        (dict(message=SimpleNamespace(document=None)), dict(user_data=MappingProxyType({}), _user_data={})),
    ],
)
