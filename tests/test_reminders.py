...
def test_render_reminders_formatting(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert [btn.text for btn in rows[0]] == ["✏️", "🗑️", "🔔"]
    assert rows[0][0].web_app is not None
    assert rows[0][0].web_app.url == config.build_ui_url("/reminders?id=1")
    assert [btn.text for btn in rows[-1]] == ["➕ Добавить"]
    add_btn = rows[-1][0]
...
def test_render_reminders_no_webapp(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    first_row, add_row = markup.inline_keyboard
