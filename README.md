...
4. **Скопируйте шаблон .env и заполните своими данными:**
   ...
   Для работы WebApp задайте `WEBAPP_URL` (например `http://localhost:8000/`).
...
Переменная окружения `WEBAPP_URL` должна указывать на адрес, по которому страница доступна пользователю. В Docker‑образе WebApp запускается автоматически вместе с ботом и доступен на порту `8000`.
