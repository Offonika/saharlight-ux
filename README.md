...
SPA будет доступна по адресу `/ui` и автоматически отправит часовой пояс на endpoint `POST /api/timezone` в формате `{"tz": "Europe/Moscow"}`. В ответ сервер возвращает `{ "status": "ok" }` и сохраняет значение в `webapp/timezone.txt`.
...
> Предыдущая статическая страница `webapp/timezone.html` устарела и приведена только для справки.
