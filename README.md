...
4. **Скопируйте шаблон .env и заполните своими данными:**
   ...
   Для работы WebApp задайте `WEBAPP_URL`. Telegram‑клиенты не могут обращаться к `localhost`, поэтому страница должна быть доступна по публичному **HTTPS**‑адресу, например `https://your-domain.example/`. Для локальной разработки используйте туннель (см. ниже).
...
Переменная окружения `WEBAPP_URL` должна указывать на HTTPS‑адрес, по которому страница доступна пользователю (Telegram принимает только HTTPS). В Docker‑образе WebApp запускается автоматически вместе с ботом и доступен на порту `8000`.
