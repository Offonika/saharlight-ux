...
а результат сборки — в `dist/`. Он определяет часовой пояс пользователя и
отправляет его на сервер.

1. **Соберите интерфейс**:

   Для разработки можно включить авто‑сборку, чтобы `dist/` обновлялся при
   изменении файлов в `src/`:
   ...
   В продакшене выполните одноразовую сборку:
