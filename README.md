...
Для диагностики можно безопасно включать `DEBUG`-уровень: фильтр логов
автоматически скрывает токен бота в сообщениях `httpx`, `telegram.ext` и
`telegram.request`, заменяя его на `bot<hidden>`. Это позволяет исследовать
обращения к Bot API без риска утечки учётных данных.
...
## Изолированный E2E тест напоминаний
В репозитории есть сценарий `tests/e2e_reminder_flow.py`,
который запускает Playwright на локальной HTML‑странице без
обращений к внешним ресурсам. Он последовательно добавляет,
редактирует и удаляет напоминание.

Запуск:

```bash
python tests/e2e_reminder_flow.py
```
