...
Сервис хранит минимум персональных данных. В таблицах памяти ассистента и журналов
обучения сохраняются только `user_id` и при необходимости ссылка на профиль. Сырые
тексты чатов не записываются и не отправляются в базу данных.
