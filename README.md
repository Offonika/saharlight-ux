...
### Восстановление после зависаний GPT

Если бот перезапустился во время анализа фото, в памяти мог остаться флаг
`waiting_gpt_response`, блокирующий обработку новых снимков. При старте бот
очищает этот флаг у всех пользователей и периодически повторяет очистку. Сам
флаг содержит метку времени и сбрасывается автоматически, если ожидание длится
более двух минут.
