...
def _build_ui_url(settings: config.Settings, path: str) -> str:
    """Return absolute UI URL for ``path`` using given ``settings``."""
...
def _render_reminders(
    session: Session, user_id: int, settings: config.Settings
    ...
    if public_origin:
        add_button_row = [
            InlineKeyboardButton(
                "➕ Добавить",
                web_app=WebAppInfo(_build_ui_url(settings, "/reminders/new")),
            )
        ]
    if not rems:
        text = header
        ...
        if add_button_row is not None:
            row.append(
                InlineKeyboardButton(
                    "✏️",
                    web_app=WebAppInfo(
                        _build_ui_url(settings, f"/reminders?id={r.id}")
                    ),
                )
            )
        row.extend(
...
async def reminders_list(
    ...
    *,
    ...
    render_fn = cast(
        ...
        partial(_render_reminders, settings=settings),
    )
    if run_db is None:
        with SessionLocal() as session:
