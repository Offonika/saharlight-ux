...
async def profile_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    * ``/profile <args>`` → set profile directly
    ...
    api, ApiException, ProfileModel = get_api(SessionLocal)
...
async def profile_view(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if message is None or user is None:
        return
    api, ApiException, _ = get_api(SessionLocal)
    user_id = user.id
            ...
            keyboard = InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "📝 Заполнить форму",
                            web_app=WebAppInfo(reminder_handlers.build_webapp_url("/ui/profile")),
                        )
                    ]
                ]
            )
