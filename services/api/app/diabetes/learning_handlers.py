...
async def _start_lesson(
    ...
    """Start a lesson and send the first step generated by the curriculum engine."""
    ...
    text, _ = await curriculum_engine.next_step(
        telegram_id, progress.lesson_id, profile
    )
    if text is None:
        return
...
async def lesson_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await _start_lesson(message, user_data, profile, topic_slug, telegram_id)
...
async def lesson_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await _start_lesson(message, user_data, profile, slug, user_id)
