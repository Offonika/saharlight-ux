...
class RESTClientObject:
    ...
    def request(
        ...
        :param post_params: request parameters for
                            ...
                            ``multipart/form-data``. For multipart requests
                            this may be either a mapping object (for example
                            ``dict``) or any non-string iterable of
                            ``(key, value)`` pairs. Items must be 2-element
                            iterables; otherwise :class:`ApiValueError` is
                            raised. Supplying any other type for
                            ``post_params`` also results in
                            :class:`ApiValueError`. Values that are
            ...
            if method in ["POST", "PUT", "PATCH", "OPTIONS", "DELETE"]:
                ...
                if not content_type or re.search("json", content_type, re.IGNORECASE):
                    ...
                    if isinstance(post_params, Mapping):
                        params_seq = post_params.items()
                    elif isinstance(post_params, Iterable) and not isinstance(
                        post_params, (str, bytes)
                    ):
                        params_seq = post_params
                    else:
                        raise ApiValueError(
                            "post_params must be a mapping or non-string iterable of (key, value) pairs",
                        )
                    fields = []
                    for item in params_seq:
